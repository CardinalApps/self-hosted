...
/**
 * Returns `numColors` dominant hex colors from an image, by k-means clustering
 * a downscaled copy of it. Resolves to an empty array if the image can't be
 * read.
 */
export function sampleImageColors(src: string, numColors: number): Promise<string[]> {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      resolve(pickDominant(pixels, numColors))
    }
 ...
 */
export function pickDominant(pixels: [number, number, number][], n: number): string[] {
  if (pixels.length === 0) return []

  // Seed centroids evenly from the pixel list
  const centroids = pixels.filter((_, i) => i % Math.max(1, Math.floor(pixels.length / n)) === 0).slice(0, n)
  ...
  for (let iter = 0; iter < 10; iter++) {
    ...
    for (const px of pixels) {
      ...
      for (let i = 0; i < current.length; i++) {
        const d = colorDist(px, current[i])
  ...
  return current.map((c) => toHex(boostSaturation(c, SATURATION_BOOST)))
}
